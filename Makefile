...
test-e2e: generate fmt vet ginkgo ## Run end-to-end tests
	USE_EXISTING_CLUSTER=true NAME_PREFIX=$(NAME_PREFIX) $(GINKGO) --vv --progress --timeout 120m --coverprofile cover.out ./test/e2e -- -namespace "$(NAMESPACE)" -eventually-timeout 30m  -delete-timeout 20m $(GO_TEST_FLAGS)
